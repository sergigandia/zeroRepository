        ...
        public static void Do()
        {
            try
            {
                ...
                if (Program.Q.IsReady() && useQ)
                {
                    ...
                    {
                        if (Program.menu.Item("lpassive").GetValue<bool>())
                            Utility.DelayAction.Add(100, () => Data.castSpell(Program.Q, "lpassive", MinionN));
                        else
                ...
                {
                    if (Program.menu.Item("lpassive").GetValue<bool>())
                        Utility.DelayAction.Add(100, () => Data.castSpell(Program.W, "lpassive", Program.Player));
                    else
                        Data.castSpell(Program.W, "lpassive", Program.Player);
                }
                ...
                if (Program.E.IsReady() && useE)
                {
                    if (MinionNe.Count >= Program.menu.Item("LMinE").GetValue<Slider>().Value)
                    {
                        if (Program.menu.Item("lpassive").GetValue<bool>())
                            ...
                            Utility.DelayAction.Add(100, () => Data.castSpell(Program.E, "lpassive"));
