...
namespace MasterOfWind
{
    class Program : Modes
    {
        ...
        public float GetMainComboDamage(Obj_AI_Base target)
        {
            ...
            if (base.getSkills().getQ().IsReady())
            {
                damage += (float)player.GetSpellDamage(target, SpellSlot.Q) * 2;

            }
            if (base.getSkills().getE().IsReady())
            {
                damage +=(float) player.GetSpellDamage(target, SpellSlot.E);
            }
            if (base.getSkills().getR().IsReady())
            {
                damage += (float)player.GetSpellDamage(target, SpellSlot.R);
            }
            return damage;
        }
        public void draw(EventArgs args)
        {
            ...
            if (menu.Item("Draw R Range").GetValue<bool>())
                ...
                Utility.HpBarDamageIndicator.DamageToUnit = GetMainComboDamage;
